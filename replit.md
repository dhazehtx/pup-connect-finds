...
-   **Stripe Connect Integration**: Manages monetization, subscriptions, and commission tracking, with Stripe account data synced across provider and profile tables. Escrow/deals system handles deposit+balance payments for dog sales with webhook-driven state transitions (RESERVED→DEPOSIT_PAID→PAID_IN_FULL→DELIVERED→CONFIRMED→RELEASED), dispute resolution, and admin management.
