...
-   **Admin & Moderation**: Dedicated admin panel with logging, user reporting, abuse protection, monitoring, and a comprehensive provider application review system. An admin dashboard provides metrics, user management, orders, analytics, and settings, with login tracking for active user metrics. Admin Console at /admin/console with 5 tabs: Reports (resolve with dismiss/warn/remove/ban + confirm modal), Blocks (lookup by userId + admin unblock), Media (orphan scan with storage check + sweep), Rate Limits (live stats per route), Trash (soft-deleted posts/listings/media with restore, purge, and bulk actions). All admin routes gated by requireAdmin middleware with [PROOF:ADMIN:GATE] logging.
-   **Soft-Delete (Trash) System**: Posts, listings, and media use soft-delete with `deleted_at`, `deleted_by`, `delete_reason` columns. Media assets also have `purge_after` (30-day auto-expire). All read queries exclude soft-deleted records. Admin can restore or permanently purge trashed items. Proof logs: [PROOF:TRASH:POST], [PROOF:TRASH:LISTING], [PROOF:RESTORE:POST], [PROOF:RESTORE:LISTING], [PROOF:ADMIN:TRASH:LIST], [PROOF:ADMIN:TRASH:PURGE], [PROOF:ADMIN:TRASH:RESTORE].
...
-   **Stripe Connect Integration**: Manages monetization, subscriptions, and commission tracking, with Stripe account data synced across provider and profile tables. Escrow/deals system handles deposit+balance payments for dog sales with webhook-driven state transitions (RESERVED→DEPOSIT_PAID→PAID_IN_FULL→DELIVERED→CONFIRMED→RELEASED), dispute resolution, and admin management. Cart checkout flow: POST /api/checkout/session creates orders + Stripe Checkout Sessions from cart items, webhook at /api/webhooks/stripe handles checkout.session.completed to mark orders paid and decrement inventory, GET /api/checkout/status verifies order by session_id. Proof logs: [PROOF:CHECKOUT:*], [PROOF:WEBHOOK:CHECKOUT_COMPLETED].
-   **Escrow/Deals System**: Tables: `deals`, `deal_payments`, `deal_payouts`, `deal_disputes`, `stripe_customers`. Routes at `/api/deals/*` for deposit, balance, handoff-code, mark-delivered, confirm-received, release, dispute, refund, cancel, resolve. Admin routes at `/api/deals/admin/*`. Webhook handler at `server/routes/stripe/webhook.ts` processes `payment_intent.succeeded/failed` for deal state transitions. Dev test harness at `/api/dev/stripe/self-test` and `/api/dev/stripe/webhook-replay`. Proof logs: [PROOF:DEAL:*], [PROOF:PAYMENT:*], [PROOF:PAYOUT:*], [PROOF:WEBHOOK:*], [PROOF:STRIPE:SELFTEST].
