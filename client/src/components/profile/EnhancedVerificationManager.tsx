...
import { z } from 'zod';
import { Shield, CheckCircle, Clock, X, Award, Loader2 } from 'lucide-react';
...
interface EnhancedVerificationManagerProps {
  isVerified: boolean;
  verificationRequests: VerificationRequest[];
  onSubmitVerification: (data: VerificationFormData) => void;
}

const EnhancedVerificationManager = ({ isVerified, verificationRequests: initialRequests, onSubmitVerification }: EnhancedVerificationManagerProps) => {
  const { user } = useAuth();
  ...
  const onSubmit = async (data: VerificationFormData) => {
    ...
    try {
      ...
      form.reset();
      setDocuments({ business_license: '', id_document: '', address_proof: '' });
      setIsOpen(false);
      fetchVerificationRequests();
...
export default EnhancedVerificationManager;
